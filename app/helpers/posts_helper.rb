...
module PostsHelper
  ...
  def hide_from_timeline_for(_resource)
    link_to 'Hide from timeline', '#',
            class: 'dropdown-item'
  end

  def change_post_date_for(_resource)
    link_to 'Change date', '#',
            class: 'dropdown-item'
  end
  ...
  def show_total_post_likes_for(post)
    return "#{image_tag 'icon-like.png'}
    #{post.likes.count}".html_safe if post.likes.any?
